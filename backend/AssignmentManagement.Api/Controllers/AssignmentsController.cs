...
public class AssignmentsController : ControllerBase
{
    ...
    private readonly IAssignmentRoadmapCohereService _roadmapCohere;
    ...
    public AssignmentsController(IAssignmentService assignments, IDocumentExtractService documentExtract, IWebHostEnvironment env, IRoutineService routine, IAssignmentRoadmapGeminiService roadmapGemini, IAssignmentRoadmapOpenAiService roadmapOpenAi, IAssignmentRoadmapOpenRouterService roadmapOpenRouter, IAssignmentRoadmapGroqService roadmapGroq, IAssignmentRoadmapCohereService roadmapCohere, IAssignmentAssistantService assistant, IAssignmentChatService chat, IAssignmentExtraService extra, IAssignmentAiResponseService aiResponse)
    {
    ...
    public async Task<IActionResult> GenerateRoadmap(int id)
    {
        ...
        if (string.IsNullOrWhiteSpace(roadmapJson))
        {
            try
            {
                roadmapJson = await _roadmapCohere.GenerateRoadmapJsonAsync(pdfText, routineSummary, deadline);
            }
            catch (Exception ex) { lastError = lastError ?? ex.Message; }
        }
        ...
        return BadRequest(new { message = lastError ?? "AI could not generate roadmap. Add at least one API key: Gemini, OpenAI, OpenRouter, Groq, or Cohere in appsettings." });
    }
