...
/// <summary>AI Roadmap generation: PDF text + Routine + Deadline → JSON array of steps (step, description, startDate, endDate). Excludes 10 AM - 5 PM job hours.</summary>
public interface IAssignmentRoadmapGeminiService
{
    /// <summary>Returns roadmap JSON or null; on API failure throws with message for client.</summary>
...
public class AssignmentRoadmapGeminiService : IAssignmentRoadmapGeminiService
{
    ...
    public async Task<string?> GenerateRoadmapJsonAsync(string pdfText, string routineSummary, DateTime deadline, CancellationToken cancellationToken = default)
    {
        ...
        var systemPrompt = $@"You are an academic strategist. Based on this Assignment Text: {assignmentText}, Deadline: {deadlineStr}, and the Student's Daily Routine: {routineSummary}, generate a step-by-step roadmap. Strictly exclude working hours (10 AM - 5 PM) as the user is at a job. Output MUST be a raw JSON array of objects only, no other text: [{{ ""step"": string, ""description"": string, ""startDate"": ""ISO8601"", ""endDate"": ""ISO8601"" }}]. Do not include markdown or code blocks.";
