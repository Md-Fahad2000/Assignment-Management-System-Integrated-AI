...
public interface IAssignmentRoadmapCohereService
{
...
public class AssignmentRoadmapCohereService : IAssignmentRoadmapCohereService
{
    private readonly IConfiguration _config;
    private readonly IHttpClientFactory _httpFactory;
    private readonly ILogger<AssignmentRoadmapCohereService> _logger;

    public AssignmentRoadmapCohereService(IConfiguration config, IHttpClientFactory httpFactory, ILogger<AssignmentRoadmapCohereService> logger)
    {
        _config = config;
...
Strictly exclude working hours (10 AM - 5 PM) as the user is at a job. Output MUST be a raw JSON array of objects only, no other text: [{{ ""step"": string, ""description"": string, ""startDate"": ""ISO8601"", ""endDate"": ""ISO8601"" }}]. Use ISO8601 dates in YYYY-MM-DD or full ISO format. Do not include markdown or code blocks.";
        ...
        {
            _logger.LogWarning("Cohere generate roadmap failed: {Status} {Body}", response.StatusCode, resJson.Length > 300 ? resJson.Substring(0, 300) + "..." : resJson);
            return null;
        }

        var text = CohereResponseParser.ExtractAssistantText(resJson);
