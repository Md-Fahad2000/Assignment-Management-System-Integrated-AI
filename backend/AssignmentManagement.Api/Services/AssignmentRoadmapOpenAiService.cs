...
/// <summary>ChatGPT (OpenAI) for assignment roadmap: same JSON format as Gemini (step, description, startDate, endDate).</summary>
public interface IAssignmentRoadmapOpenAiService
{
...
public class AssignmentRoadmapOpenAiService : IAssignmentRoadmapOpenAiService
{
    private readonly IConfiguration _config;
    private readonly IHttpClientFactory _httpFactory;
    private readonly ILogger<AssignmentRoadmapOpenAiService> _logger;

    public AssignmentRoadmapOpenAiService(IConfiguration config, IHttpClientFactory httpFactory, ILogger<AssignmentRoadmapOpenAiService> logger)
    {
        _config = config;
    ...
    public async Task<string?> GenerateRoadmapJsonAsync(string pdfText, string routineSummary, DateTime deadline, CancellationToken cancellationToken = default)
    {
        ...
        {
            _logger.LogWarning("OpenAI generate roadmap failed: {Status} {Body}", response.StatusCode, resJson.Length > 300 ? resJson.Substring(0, 300) + "..." : resJson);
            return null;
        }
