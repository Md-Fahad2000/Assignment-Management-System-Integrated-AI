...
public class AssignmentAssistantService : IAssignmentAssistantService
{
    ...
    private async Task<string?> CallCohereAsync(List<object> messages, CancellationToken cancellationToken)
    {
        ...
        {
            _logger.LogWarning("Cohere assistant failed: {Status} {Body}", response.StatusCode, resJson.Length > 400 ? resJson.Substring(0, 400) + "..." : resJson);
            return null;
        }
