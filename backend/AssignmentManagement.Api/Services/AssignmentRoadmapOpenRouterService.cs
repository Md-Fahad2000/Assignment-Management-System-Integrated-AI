...
/// <summary>OpenRouter free models (openrouter/free) for assignment roadmap - same JSON format. Good for testing when Gemini/OpenAI quota exceeded.</summary>
public interface IAssignmentRoadmapOpenRouterService
{
...
public class AssignmentRoadmapOpenRouterService : IAssignmentRoadmapOpenRouterService
{
    private readonly IConfiguration _config;
    private readonly IHttpClientFactory _httpFactory;
    private readonly ILogger<AssignmentRoadmapOpenRouterService> _logger;

    public AssignmentRoadmapOpenRouterService(IConfiguration config, IHttpClientFactory httpFactory, ILogger<AssignmentRoadmapOpenRouterService> logger)
    {
        _config = config;
    ...
    public async Task<string?> GenerateRoadmapJsonAsync(string pdfText, string routineSummary, DateTime deadline, CancellationToken cancellationToken = default)
    {
        ...
        {
            model = _config["OpenRouter:Model"] ?? "openrouter/free",
        ...
        {
            _logger.LogWarning("OpenRouter generate roadmap failed: {Status} {Body}", response.StatusCode, resJson.Length > 300 ? resJson.Substring(0, 300) + "..." : resJson);
            return null;
        }
