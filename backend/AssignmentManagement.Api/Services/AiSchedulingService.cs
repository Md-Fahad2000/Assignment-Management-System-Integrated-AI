...
/// <summary>
/// AI Scheduling: Earliest-Deadline-First (EDF) with routine constraints.
/// Assigns study slots for assignments in free time windows, respecting existing routine.
/// </summary>
public class AiSchedulingService : IAiSchedulingService
{
    ...
    private const int DefaultSlotMinutes = 120; // 2-hour study blocks
    /// <summary>Clear actionable steps shown when no AI is used.</summary>
    ...
    /// <summary>How to do each step (for fallback when AI is not used).</summary>
    ...
    /// <summary>Generate AI schedule: EDF with routine constraints. Fills ai_schedules for the user.</summary>
    ...
    /// <summary>AI Assignment Roadmap: only from AI (OpenAI or Gemini). No fallback – if AI returns nothing, roadmap stays empty.</summary>
    ...
    /// <summary>List of (date, time) slots from today to deadline, using ONLY free windows (never routine/busy time).</summary>
    ...
    /// <summary>Free windows as (date, startTime, endTime). ONLY free time (routine = busy, never used). Split into 1-hour slots so each step gets a unique slot and no duplicate date/time.</summary>
    ...
    /// <summary>Split document text into logical parts (by headings, numbered items, or paragraphs).</summary>
    ...
    /// <summary>Get list of dates from today to deadline, considering routine (prefer days with more free time).</summary>
    ...
    /// <summary>Stream roadmap from Gemini (live AI response), then parse and save. Calls onChunk for each text chunk.</summary>
    ...
    /// <summary>Get the context used to build the roadmap: assignment (title, deadline, description, document) and routine summary.</summary>
